...
client.obtenerProducto({ id: 1 }, (err, producto) => {
    ...
    client.obtenerProducto({ id: 999 }, (err, producto) => {
        ...
        console.log('\n== BuscarPorPrecioMaximo (server streaming <= 50) ==');
        const searchCall = client.buscarPorPrecioMaximo({ precioMaximo: 50 });
        searchCall.on('data', (p) => {
            console.log(`Búsqueda: ${p.id} - ${p.nombre} - $${p.precio}`);
        });
        searchCall.on('end', () => console.log('Búsqueda finalizada.'));
        searchCall.on('error', (err) => console.error('Error en búsqueda:', err.code, err.details));
    });
});